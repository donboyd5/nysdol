...
# Get and save QCEW QUARTERLY data (later update to get annual too)
...
constants$nyc <- constants$mtaregion[1:5]
...
```{r}
#| label: check-recompression-needed

zpath <- fs::path(DRAW, "qcew.zip")
...
# Pipe directly into vroom - if this works, recompression not needed
...
## Save state and county subset

```{r}
#| label: clean-save-stateco
#| eval: false

system.time(qcew <- readRDS(fs::path(DDATA, "qcew_clean.rds"))) # 14 secs

count(qcew, areatype)
qcew_stateco <- qcew |>
  filter(areatype %in% c("state", "county"))

system.time(saveRDS(qcew_stateco, fs::path(DDATA, "qcew_stateco.rds"))) # 23 secs

```

## Save MTA (plus NYS) subset  

```{r}
#| label: clean-save-mta
#| eval: false
...
glimpse(qmta)
count(qmta, areatype, mta, area)

system.time(saveRDS(qmta, fs::path(DDATA, "qcew_statemta.rds"))) # 9 secs
...
system.time(qmta <- readRDS(fs::path(DDATA, "qcew_statemta.rds")))

glimpse(qcew)
count(qcew, areatype)
count(qcew, owner)
count(qcew |> filter(as.integer(owner) == 4), owner)
